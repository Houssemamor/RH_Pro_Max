...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    // Note: the role is an enum (UserRole), NOT a string
    ...
    // Important: the type is ?UserRole
    ...
    // Roles for Symfony Security
    public function getRoles(): array
    {
        $roles = [];

        if ($this->role !== null) {
            // enum -> text (ADMIN, HR_MANAGER, ...)
            $roles[] = 'ROLE_' . $this->role->value;
        }

        // everyone has at least ROLE_USER
        $roles[] = 'ROLE_USER';
